...
export default function Dashboard() {
  ...
  const callEndpoint = async (path: string) => {
    setLoadingPath(path);
    try {
      const data = await testEndpoint(path, user.token);
      setResults((prev) => [
        { path, response: data.message || JSON.stringify(data), ok: true },
        ...prev.filter((r) => r.path !== path),
      ]);
    } catch (err: unknown) {
      setResults((prev) => [
        { path, response: err instanceof Error ? err.message : "Request failed", ok: false },
        ...prev.filter((r) => r.path !== path),
      ]);
    } finally {
      setLoadingPath(null);
